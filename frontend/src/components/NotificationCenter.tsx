...
import { BellIcon, CheckCircleIcon, MessageSquareIcon, CalendarIcon } from './Icons.tsx';
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-bg-main dark:bg-surface-dark rounded-lg shadow-xl border border-border-main dark:border-border-dark z-50 animate-fade-in-down">
          <div className="p-3 border-b border-border-main dark:border-border-dark">
            <h3 className="font-semibold text-main dark:text-main">Notificaciones</h3>
          </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-grow">
                        <p className="text-sm text-main dark:text-main">{n.message}</p>
                        <span className="text-xs text-muted dark:text-muted">{n.createdAt.toLocaleString('es-ES', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'})}</span>
                      </div>
                      {n.status === 'unread' && (
