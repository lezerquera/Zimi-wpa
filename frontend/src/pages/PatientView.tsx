...
import type { DoctorProfile, Service, Appointment, ChatMessage, ClinicInfo, Insurance, User, FormTemplate, PatientFormSubmission, Question, ClinicalWizardAnswers, BodyPainPoint, QuestionType } from '../types.ts';
import { 
...
import { generateChatSummary } from '../services/geminiService.ts';
import { InsuranceCarousel } from '../components/InsuranceCarousel.tsx';

const API_BASE_URL = import.meta.env.VITE_API_URL || '/api';

// --- START OF BODY MAP SVG COMPONENTS ---
// Moved before usage to fix reference errors
...
// Create a reverse map for efficient lookup to highlight general areas from specific pain points.
const REVERSE_AREAS_MAP: Record<string, string> = Object.entries(SPECIFIC_AREAS_MAP).reduce((acc, [general, specifics]) => {
...
const BodyPart = ({ id, d, title, selectedParts, onClick }: { 
    id: string; 
    d: string; 
    title?: string;
    ...
    const isSelected = selectedParts.some(partName => {
        const generalPartForSelected = REVERSE_AREAS_MAP[partName];
        return generalPartForSelected === id || partName === id;
    });
    return (
        <path
            id={id}
...
const BodyMapFront = ({ onClick, selectedParts }: { onClick: (part: string) => void; selectedParts: string[] }) => {
    return (
        <svg viewBox="0 0 300 600" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-[250px] mx-auto">
            <g id="body-front">
                <BodyPart id="cabeza" title="Cabeza" d="M150,52c-20.5,0-37-16.5-37-37s16.5-37,37-37,37,16.5,37,37-16.5,37-37,37Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="cuello" title="Cuello" d="M132,70c0,14,3,22,18,22s18-8,18-22c0-6-36-6-36,0Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="hombro-izquierdo" title="Hombro izquierdo" d="M92,100c-14,0-26-10-28-24l-.6-4.2c-1.5-10.2,7.7-19,18-17.1,13.6,2.3,24.6,19.3,28.6,36.3,1.7,7-7,9-18,9Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="hombro-derecho" title="Hombro derecho" d="M208,100c14,0,26-10,28-24l.6-4.2c1.5-10.2-7.7-19-18-17.1-13.6,2.3-24.6,19.3-28.6,36.3-1.7,7,7,9,18,9Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pecho" title="Pecho" d="M110,100c-9,0-16,7.3-16,16.3,0,13.1,2.6,25.4,6.4,34.7,5.6,13.8,16.3,19,49.6,19s44-5.2,49.6-19c3.8-9.3,6.4-21.6,6.4-34.7,0-9-7-16.3-16-16.3H110Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="abdomen" title="Abdomen" d="M118,170c-6.5,0-12,5.2-11.7,11.7,.6,12.5,2.8,27.2,7.7,39.3,6.9,17.3,19.5,23.7,35.9,23.7s29-6.4,35.9-23.7c4.9-12.1,7.1-26.8,7.7-39.3,.3-6.5-5.2-11.7-11.7-11.7H118Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="brazo-superior-izquierdo" title="Brazo Superior Izquierdo" d="M81,128c-8.3,0-15,11.2-15,25v50c0,13.8,6.7,25,15,25s15-11.2,15-25v-50c0-13.8-6.7-25-15-25Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="brazo-superior-derecho" title="Brazo Superior Derecho" d="M219,128c8.3,0,15,11.2,15,25v50c0,13.8-6.7,25-15,25s-15-11.2-15-25v-50c0-13.8,6.7-25,15-25Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="antebrazo-izquierdo" title="Antebrazo Izquierdo" d="M79,229c-7.2,0-13,9-13,20v45c0,11,5.8,20,13,20s13-9,13-20v-45c0-11-5.8-20-13-20Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="antebrazo-derecho" title="Antebrazo Derecho" d="M221,229c7.2,0,13,9,13,20v45c0,11-5.8,20-13,20s-13-9-13-20v-45c0-11,5.8-20,13-20Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="mano-izquierda" title="Mano Izquierda" d="M76,315c-6.1,0-11,8.1-11,18s4.9,18,11,18,11-8.1,11-18-4.9-18-11-18Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="mano-derecha" title="Mano Derecha" d="M224,315c6.1,0,11,8.1,11,18s-4.9,18-11,18-11-8.1-11-18,4.9-18,11-18Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="muslo-izquierdo" title="Muslo Izquierdo" d="M110,326c-8.3,0-15,17.9-15,40v45c0,22.1,6.7,40,15,40s15-17.9,15-40v-45c0-22.1-6.7-40-15-40Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="muslo-derecho" title="Muslo Derecho" d="M190,326c8.3,0,15,17.9,15,40v45c0,22.1-6.7,40-15,40s-15-17.9-15-40v-45c0-22.1,6.7-40,15-40Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pierna-inferior-izquierda" title="Pierna Inferior Izquierda" d="M112,452c-6.6,0-12,13.4-12,30s5.4,30,12,30,12-13.4,12-30-5.4-30-12-30Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pierna-inferior-derecha" title="Pierna Inferior Derecha" d="M188,452c6.6,0,12,13.4,12,30s-5.4,30-12,30-12-13.4-12-30,5.4-30-12-30Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pie-izquierdo" title="Pie Izquierdo" d="M109,515c-8.3,0-15,11.2-15,25s6.7,25,15,25h20c8.3,0,15-11.2,15-25s-6.7-25-15-25h-20Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pie-derecho" title="Pie Derecho" d="M171,515c8.3,0,15,11.2,15,25s-6.7,25-15,25h-20c-8.3,0-15-11.2-15-25s6.7-25,15-25h20Z" onClick={onClick} selectedParts={selectedParts} />
            </g>
        </svg>
    );
};
...
const BodyMapBack = ({ onClick, selectedParts }: { onClick: (part: string) => void; selectedParts: string[] }) => {
    return (
        <svg viewBox="0 0 300 600" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-[250px] mx-auto">
            <g id="body-back">
                <BodyPart id="cabeza" title="Nuca / Occipucio" d="M150,52c-20.5,0-37-16.5-37-37s16.5-37,37-37,37,16.5,37,37-16.5,37-37,37Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="cuello-posterior" title="Cuello (Posterior)" d="M132,70c0,14,3,22,18,22s18-8,18-22c0-6-36-6-36,0Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="espalda-superior" title="Espalda Superior" d="M109,92.5c-15.5,0-28,15.7-28,35v45h138v-45c0-19.3-12.5-35-28-35H109Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="espalda-inferior" title="Espalda Inferior / Lumbar" d="M115.1,173.5c-4.4,0-8,4.5-8,10v60c0,5.5,3.6,10,8,10h69.8c4.4,0,8-4.5,8-10v-60c0-5.5-3.6-10-8-10H115.1Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="gluteos" title="Glúteos" d="M109.1,255.5c-11.1,0-20,15.7-20,35s8.9,35,20,35h81.8c11.1,0,20-15.7,20-35s-8.9-35-20-35H109.1Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="brazo-superior-izquierdo" title="Brazo Superior Izquierdo (Posterior)" d="M81.1,128.5c-8.3,0-15,11.2-15,25v50c0,13.8,6.7,25,15,25h0c8.3,0,15-11.2,15-25v-50c0-13.8-6.7-25-15-25Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="brazo-superior-derecho" title="Brazo Superior Derecho (Posterior)" d="M218.9,128.5c8.3,0,15,11.2,15,25v50c0,13.8-6.7,25-15,25h0c-8.3,0-15-11.2-15-25v-50c0-13.8,6.7-25,15-25Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="antebrazo-izquierdo" title="Antebrazo Izquierdo (Posterior)" d="M79.1,229.5c-7.2,0-13,9-13,20v45c0,11,5.8,20,13,20s13-9,13-20v-45c0-11-5.8-20-13-20Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="antebrazo-derecho" title="Antebrazo Derecho (Posterior)" d="M220.9,229.5c7.2,0,13,9,13,20v45c0,11-5.8,20-13,20s-13-9-13-20v-45c0-11,5.8-20,13-20Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="muslo-izquierdo" title="Muslo Izquierdo (Posterior)" d="M110.1,326.5c-8.3,0-15,17.9-15,40v45c0,22.1,6.7,40,15,40s15-17.9,15-40v-45c0-22.1-6.7-40-15-40Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="muslo-derecho" title="Muslo Derecho (Posterior)" d="M189.9,326.5c8.3,0,15,17.9,15,40v45c0,22.1-6.7,40-15,40s-15-17.9-15-40v-45c0-22.1,6.7-40,15-40Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pierna-inferior-izquierda" title="Pierna Inferior Izquierda (Posterior)" d="M112.1,452.5c-6.6,0-12,13.4-12,30s5.4,30,12,30,12-13.4,12-30-5.4-30-12-30Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pierna-inferior-derecha" title="Pierna Inferior Derecha (Posterior)" d="M187.9,452.5c6.6,0,12,13.4,12,30s-5.4,30-12,30-12-13.4-12-30,5.4-30,12-30Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pie-izquierdo" title="Pie Izquierdo (Posterior)" d="M109.1,515.5c-8.3,0-15,11.2-15,25s6.7,25,15,25h20c8.3,0,15-11.2,15-25s-6.7-25-15-25h-20Z" onClick={onClick} selectedParts={selectedParts} />
                <BodyPart id="pie-derecho" title="Pie Derecho (Posterior)" d="M170.9,515.5c-8.3,0-15,11.2-15,25s6.7,25,15,25h20c8.3,0,15-11.2,15-25s-6.7-25-15-25h-20Z" onClick={onClick} selectedParts={selectedParts} />
            </g>
        </svg>
    );
};
...
// --- END OF BODY MAP SVG COMPONENTS ---
