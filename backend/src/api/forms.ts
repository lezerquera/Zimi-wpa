...
const calculatePriority = (answers: ClinicalWizardAnswers): 'high' | 'medium' | 'low' => {
    ...
    if (answers.bodyMap && answers.bodyMap.some((point: BodyPainPoint) => point.intensity >= 8)) {
        return 'high';
    }
    // Rule 2: Dolor lumbar severo -> Alta prioridad
    if (answers.bodyMap && answers.bodyMap.some((point: BodyPainPoint) => point.bodyPart === 'lumbar' && point.intensity > 7)) {
        return 'high';
    }

    // Rule 3: Dolor moderado -> Prioridad moderada
    if (answers.bodyMap && answers.bodyMap.some((point: BodyPainPoint) => point.intensity >= 5)) {
        return 'medium';
    }
